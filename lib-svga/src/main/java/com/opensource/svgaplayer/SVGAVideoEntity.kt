...
class SVGAVideoEntity {
    ...
    private var mPlayCallbackRef: WeakReference<SVGAParser.PlayCallback>? = null
    private var mCallbackRef: WeakReference<() -> Unit>? = null
    ...
    internal fun prepare(callback: () -> Unit, playCallback: SVGAParser.PlayCallback?) {
        ...
        mPlayCallbackRef = WeakReference(playCallback)
        if (movieItem == null) {
            mCallbackRef?.get()?.invoke()
        } else {
